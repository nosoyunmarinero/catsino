...
export const SlotMachine = ({ onBack }) => {
  const { balance, turboMode, setTurboMode, claimFreeCoins, freeSpinsLeft } = useCasinoStore();
  ...
  return (
    <div className="slot-cabinet" style={{
      ...
      <button 
        onClick={onBack}
        style={{
          position: 'absolute', top: '10px', left: '10px', background: '#333',
          color: '#fff', border: '2px solid var(--gold)', borderRadius: '50%', 
          width: '30px', height: '30px', fontSize: '1rem', cursor: 'pointer', zIndex: 5,
      ...
      <button 
        ...
        style={{
          ...
          fontFamily: 'var(--font-display)', fontSize: '1rem', cursor: 'pointer', zIndex: 5,
      ...
      <div style={{
        ...
        <div>
          <label style={{ display: 'block', fontSize: '0.75rem', marginBottom: '5px', color: 'var(--gold)' }}>
            Bet/Line:
          </label>
        ...
        <div>
          ...
          <input
            type="range" min="1" max="20" value={activeLines}
            disabled={isAnyReelSpinning || isAutoActive}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', justifyContent: 'center' }}>
          ...
          {balance <= 0 && (
            <Button variant="danger" onClick={claimFreeCoins} style={{ fontSize: '0.7rem', padding: '8px' }}>
              🐟 Auxilio: +1000
            </Button>
          )}
        </div>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'left' }}>
          ...
          <h3 style={{ color: 'var(--salmon)', fontFamily: 'var(--font-display)', margin: 0, fontSize: '1.1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flex: '1', justifyContent: 'flex-end' }}>
          {isAutoActive ? (
            <Button variant="danger" onClick={stopAuto} style={{ flex: 1, padding: '10px' }}>
              🛑 Stop ({remainingSpins})
            </Button>
          ) : (
            <Button
